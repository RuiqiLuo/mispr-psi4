...
class LinkedMolPsi4FW(Firework):
    """
    Combine two previously-computed molecules into one (forming a bond at given
    sites) and immediately optimize the resulting complex with psi4.

    ``ProcessMoleculeInput`` (reused unmodified from the Gaussian firetasks -- it is
    plain geometry bookkeeping, not tied to any QM engine) does the linking, using
    the ``mol_operation_type="link_molecules"`` mode of
    ``mispr.gaussian.utilities.mol.process_mol``: it reads the two molecules'
    optimized geometries from ``fw_spec["gaussian_output"]`` (set by the two prior
    optimization/frequency Fireworks) and joins them at ``index`` with a bond of
    order ``bond_order``. The resulting molecule is picked up by ``RunPsi4`` via
    ``fw_spec["prev_calc_molecule"]``.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            gout_keys (list): The two keys in fw_spec["gaussian_output"] (from the
                two molecules' own optimization/frequency Fireworks) to link.
            index (list): Site indices in each molecule at which to form the bond.
            bond_order (int, optional): Bond order for the new bond; defaults to 1.
            db (str or dict, optional): Database credentials.
            name (str, optional): Name of the Firework.
            parents (Firework or [Firework], optional): Parent FWs this FW depends on.
            working_dir (str, optional): Working directory for the calculation.
            gaussian_input_params (dict, optional): Optimization parameters (see
                ``Psi4FW``).
            tag (str, optional): Tag stored in the db documents.
            gout_key (str, optional): Key to store the optimized complex under in
                fw_spec["gaussian_output"].
            filename (str, optional): Name to save the linked molecule under, if
                ``save_to_file``/``save_to_db`` is requested.
            kwargs: other kwargs passed to Firework.__init__ and RunPsi4.
        """
        working_dir = working_dir or os.getcwd()
        if not os.path.exists(working_dir):
        ...
        opt_task = RunPsi4(gout_key=gout_key, db=db, **task_kwargs)

        spec = kwargs.pop("spec", {})
        spec.update({"tag": tag, "_launch_dir": working_dir})
        super(LinkedMolPsi4FW, self).__init__(
            [link_task, opt_task],
            parents=parents,
            name=name,
            spec=spec,
